...
export default function PricingPage() {
    const plans = [
        { name: 'Free Trial', price: '$0', properties: '3 properties', description: 'Try all features for 30 days', features: ['Up to 3 properties', 'Full feature access', 'Payment tracking', 'Tenant management', 'Basic reporting', '30-day free trial', 'No credit card required'], cta: 'Start Free Trial', popular: false, trial: true, trialDays: 30 },
        { name: 'Starter', price: '$15', properties: '5 properties', description: 'Perfect for getting started', features: ['Up to 5 properties', 'Payment tracking & reminders', 'Basic financial reports', 'Email support', 'Automated late fees', 'Document storage'], cta: 'Get Started', popular: false, trial: false },
        { name: 'Professional', price: '$30', properties: '10 properties', description: 'Ideal for growing portfolios', features: ['Up to 10 properties', 'Advanced financial tracking', 'Automated late fees', 'Priority email support', 'Custom reports', 'API access'], cta: 'Get Started', popular: true, trial: false },
        { name: 'Business', price: '$60', properties: '20 properties', description: 'For established portfolios', features: ['Up to 20 properties', 'Advanced analytics', 'Phone & email support', 'API access', 'White-label reports', 'Bulk operations', 'Custom integrations'], cta: 'Get Started', popular: false, trial: false },
        { name: 'Enterprise', price: '$99', properties: 'Unlimited properties', description: 'For serious property investors', features: ['Unlimited properties', 'All Business features', 'Dedicated account manager', 'Custom integrations', 'SLA guarantee', 'Training & onboarding', 'Priority support'], cta: 'Contact Sales', popular: false, trial: false }
    ]
            ...
            <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 mb-16 sm:mb-20">
                <div className="flex flex-col lg:grid lg:grid-cols-5 gap-4 sm:gap-6 overflow-x-auto lg:overflow-visible pb-6 lg:pb-0">
                    {plans.map((plan, index) => (
                        <div
                            key={index}
                            className={`relative bg-white rounded-2xl lg:rounded-3xl border ${plan.popular ? 'border-[#876D4A] ring-2 ring-[#876D4A]/20' : 'border-gray-200'} p-4 sm:p-6 shadow-sm flex flex-col min-w-[280px] sm:min-w-[300px] lg:min-w-0`}
                        >
                            {/* Labels */}
                            {plan.popular && <div className="absolute -top-2 left-1/2 transform -translate-x-1/2 z-10">
                                <span className="bg-[#876D4A] text-white px-3 py-1 rounded-full text-xs font-medium">Most Popular</span>
                            </div>}
                            {plan.trial && <div className="absolute -top-2 left-4 z-10">
                                <span className="bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs font-medium">{plan.trialDays}-day free trial</span>
                            </div>}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
                        <div className="space-y-4 sm:space-y-6">
                            <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200">
                                ...
                                <p className="text-gray-600 text-xs sm:text-sm">Start with 30 days free with access to 3 properties. No credit card required. Upgrade to a paid plan after your trial ends.</p>
                            </div>
                            <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200">
                                <h3 className="font-medium text-gray-900 text-sm sm:text-base mb-2">What happens after the trial?</h3>
                                <p className="text-gray-600 text-xs sm:text-sm">After your 30-day trial ends, you'll need to choose a paid plan to continue using Denly. Your data is preserved.</p>
                            </div>
                        </div>
                        <div className="space-y-4 sm:space-y-6">
                            <div className="bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200">
